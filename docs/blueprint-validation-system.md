...
-   Добавляет базовые типы данных на основе `data_type` Path
-   Обрабатывает специальные случаи для массивов (`array` правило)
-   Для `data_type: 'json'` добавляет правило `JsonObject` (проверка, что это объект, а не массив)
...
-   `unique` → `UniqueRuleHandler`
-   `exists` → `ExistsRuleHandler`
...
**Описание**: Значение должно быть уникальным в указанной таблице/колонке.

**Параметры**:

-   `table` — таблица для проверки
-   `column` — колонка для проверки (по умолчанию используется имя поля)
-   `exceptColumn` / `exceptValue` — исключение при обновлении
-   `whereColumn` / `whereValue` — дополнительное WHERE условие
...
// С WHERE условием
$ruleFactory->createUniqueRule('users', 'email', null, null, 'status', 'active');
```
...
**Описание**: Значение должно существовать в указанной таблице/колонке.
...
| `json`           | `'array'` + `JsonObject` | JSON объект (массив в Laravel) |
| `ref`            | `'integer'`              | Ссылка (ID)                    |

**Особенности**:

-   Для `data_type: 'json'` с `cardinality: 'one'` добавляется правило `JsonObject` (проверка, что это объект, а не массив)
