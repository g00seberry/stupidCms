...
1. **`Entry.template_override`** — переопределение для конкретной записи (наивысший приоритет)
2. **`PostType.template`** — шаблон типа поста
3. **Резолвер шаблонов** — автоматический выбор по slug или типу поста (см. `BladeTemplateResolver`)
