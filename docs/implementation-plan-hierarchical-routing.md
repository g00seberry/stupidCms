...
1. Расширить `ReservedRouteRegistry` (если необходимо):
...
2. Создать/обновить компонент для генерации regex паттернов:

    - Метод `firstSegmentRegex(): string` — regex для первого сегмента с исключением reserved
    - Поддержка multi-level путей
    - Примечание: старый `ReservedPattern` удалён как легаси
