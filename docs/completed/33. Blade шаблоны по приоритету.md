...
2) **Шаблон пост-типа** (по соглашению пути `pages.types.{postType->slug}`) — если существует.
...
final class BladeTemplateResolver implements TemplateResolver
{
    ...
    public function forEntry(Entry $entry): string
    {
        ...
        // 2) По типу поста (slug берём из связи postType->slug)
        $typeKey = $entry->postType->slug ?? 'page';
...
- Шаблон пост-типа из БД: поле `postType.template` для явного указания шаблона (вместо соглашения по пути).
