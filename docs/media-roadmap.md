...
Реализовать управление жизненным циклом: добавить таблицу media_usages или связь с Entry, чтобы перед delete/restore контроллер мог проверять, используется ли файл, и обеспечивать каскадное обновление.
...
Отделить маршруты публичного доступа: предоставить /media/{id} с короткими подписанными URL (TTL из config), чтобы клиентам не требовалась админ‑аутентификация, и задокументировать это в docs/media-system.md.
