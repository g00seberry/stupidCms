...
1.1. Изучить текущую структуру Entry:
   ...
   - `data_json`, `seo_json`, `template_override`
...
1.3. Определить enum-значения:
   ...
   - `RouteNodeActionType`: `CONTROLLER`, `INVOKABLE`, `VIEW`, `REDIRECT`, `ENTRY`, `ENTRY_RESOLVER`
...
3.2. Создать `app/Enums/RouteNodeActionType.php`:
   - `CONTROLLER`, `INVOKABLE`, `VIEW`, `REDIRECT`, `ENTRY`, `ENTRY_RESOLVER`
   - Метод `values(): array`
   - Метод `requiresEntry(): bool` (для `ENTRY`, `ENTRY_RESOLVER`)
...
- [ ] `RouteNodeActionType::requiresEntry()` возвращает `true` для `ENTRY` и `ENTRY_RESOLVER`
...
## Блок 9: Создание базового DynamicRouteRegistrar
...
9.4. Обработка `enabled=false`:
   - Узел не регистрируется

9.5. Структурное логирование ошибок регистрации

9.6. Добавить PHPDoc

### Результаты
- Базовый регистратор маршрутов
- Поддержка групп и простых маршрутов
...
## Блок 10: Поддержка action_type: controller и invokable

### Задачи
10.1. Расширить `DynamicRouteRegistrar`:
   - Обработка `action_type=CONTROLLER`:
     - Парсинг `action` как `Controller@method`
     - Проверка через `DynamicRouteGuard`
     - Регистрация через `Route::match(..., [Controller::class, 'method'])`
   - Обработка `action_type=INVOKABLE`:
     - Парсинг `action` как полное имя класса
     - Проверка через `DynamicRouteGuard`
     - Регистрация через `Route::match(..., Controller::class)`

10.2. Обработка ошибок:
   - Неразрешённый контроллер → safe action `fn() => abort(404)`
   - Некорректный формат `action` → логирование + safe action

10.3. Добавить валидацию формата `action`:
   - `CONTROLLER`: `App\\Http\\Controllers\\TestController@method`
   - `INVOKABLE`: `App\\Http\\Controllers\\TestController`

### Результаты
- Поддержка контроллеров и invokable-классов

### Тесты после блока 10
**Feature:**
- [ ] `action_type=CONTROLLER` с `action='App\\Http\\Controllers\\TestController@show'` регистрирует маршрут
- [ ] `action_type=INVOKABLE` с `action='App\\Http\\Controllers\\TestController'` регистрирует маршрут
- [ ] Неразрешённый контроллер заменяется на safe action
- [ ] Некорректный формат `action` логируется и не ломает регистрацию

---

## Блок 11: Поддержка action_type: view и redirect

### Задачи
11.1. Расширить `DynamicRouteRegistrar`:
   - Обработка `action_type=VIEW`:
     - Использование поля `view` как имени Blade-шаблона
     - Регистрация через `Route::match(..., fn() => view($view))`
   - Обработка `action_type=REDIRECT`:
     - Использование `options->to` как целевого URL
     - Использование `options->status` как HTTP-статуса (по умолчанию 302)
     - Регистрация через `Route::match(..., fn() => redirect($to, $status))`

11.2. Валидация:
   - `VIEW`: `view` обязателен
   - `REDIRECT`: `options->to` обязателен

11.3. Поддержка `kind=REDIRECT` как альтернатива `action_type=REDIRECT` (для совместимости)

### Результаты
- Поддержка view и redirect

### Тесты после блока 11
**Feature:**
- [ ] `action_type=VIEW` с `view='pages.about'` возвращает Blade-шаблон
- [ ] `action_type=REDIRECT` с `options->to='/old'` и `options->status=301` делает редирект
- [ ] `kind=REDIRECT` работает аналогично `action_type=REDIRECT`
- [ ] Отсутствие обязательных полей логируется

---

## Блок 12: Интеграция Entry (режим A — жёсткое назначение)

### Задачи
12.1. Создать `app/Http/Controllers/EntryPageController.php`:
...
## Блок 13: Интеграция Entry (режим B — динамическое разрешение)

### Задачи
13.1. Создать `app/Http/Controllers/EntryResolverController.php`:
   - Метод `show(Request $request, string $slug): Response`
   - Получение `route_node_id` из defaults
   - Загрузка `RouteNode` и извлечение `options->post_type_id`, `options->slug_path`

13.2. Поиск Entry:
   - Использование `Entry::ofType($postTypeId)->published()->wherePath($slugPath, '=', $slug)->firstOrFail()`
   - Валидация: `post_type_id > 0`, `slug_path` не пустой

13.3. Рендеринг аналогично `EntryPageController`

13.4. В `DynamicRouteRegistrar`:
   - При `action_type=ENTRY_RESOLVER` задавать action на `EntryResolverController@show`
   - Добавлять default: `route_node_id = $node->id`

13.5. Добавить PHPDoc

### Результаты
- Один маршрут обслуживает множество Entry по slug

### Тесты после блока 13
**Feature:**
- [ ] Blueprint Path `slug` отмечен как `is_indexed=true`
- [ ] Entry сохраняется → индексы созданы
- [ ] `GET /blog/{slug}` находит опубликованную Entry через `wherePath`
- [ ] Даёт 404 при отсутствии значения или при draft
- [ ] Даёт 404 при неверном `post_type_id` или `slug_path`
- [ ] JSON-ответ содержит Entry

---

## Блок 14: Создание DynamicRouteCache

### Задачи
14.1. Создать `app/Services/DynamicRoutes/DynamicRouteCache.php`:
   - Метод `rememberTree(callable $callback): Collection`
   - Метод `forgetTree(): void`
   - Использование `Cache::remember()` с ключом из конфига

14.2. Интеграция в `RouteNodeRepository`:
   - Обернуть `getTree()` в кэш через `DynamicRouteCache`

14.3. Версионирование ключа:
...
## Блок 15: Создание RouteNodeObserver для инвалидации кэша

### Задачи
15.1. Создать `app/Observers/RouteNodeObserver.php`:
...
15.2. Зарегистрировать Observer в `AppServiceProvider`:
   - `RouteNode::observe(RouteNodeObserver::class)`

15.3. Добавить PHPDoc
...
## Блок 16: Создание Artisan-команд для управления кэшем

### Задачи
16.1. Создать `app/Console/Commands/DynamicRoutesCacheCommand.php`:
   - Команда `routes:dynamic-cache` — прогрев кэша
   - Вызов `RouteNodeRepository::getTree()` для заполнения кэша

16.2. Создать `app/Console/Commands/DynamicRoutesClearCommand.php`:
   - Команда `routes:dynamic-clear` — сброс кэша
   - Вызов `DynamicRouteCache::forgetTree()`

16.3. Зарегистрировать команды в `app/Console/Kernel.php` или через атрибуты

16.4. Добавить PHPDoc и описания команд
...
## Блок 17: Интеграция в RouteServiceProvider

### Задачи
17.1. Модифицировать `app/Providers/RouteServiceProvider.php`:
...
## Блок 18: Создание Admin API — CRUD для RouteNode

### Задачи
18.1. Создать `app/Http/Controllers/Admin/V1/RouteNodeController.php`:
...
18.2. Создать `app/Http/Resources/Admin/RouteNodeResource.php`:
...
## Блок 19: Создание FormRequest для валидации RouteNode

### Задачи
19.1. Создать `app/Http/Requests/Admin/StoreRouteNodeRequest.php`:
   - Валидация `kind` (enum)
...
19.2. Создать `app/Http/Requests/Admin/UpdateRouteNodeRequest.php`:
   - Аналогично `StoreRouteNodeRequest`, но все поля опциональны

19.3. Кастомные правила валидации:
   - Проверка `uri` на запрещённые префиксы через `DynamicRouteGuard`
   - Проверка `action` на формат для `CONTROLLER`/`INVOKABLE`
...
## Блок 21: Политика удаления и целостность дерева

### Задачи
21.1. Определить политику удаления:
...
## Блок 22: Создание Policy для RouteNode

### Задачи
22.1. Создать `app/Policies/RouteNodePolicy.php`:
...
22.2. Зарегистрировать Policy в `AuthServiceProvider`:
   - `RouteNode::class => RouteNodePolicy::class`

22.3. Использовать в контроллере:
   - `$this->authorize('manage', RouteNode::class)` или `$this->authorize('update', $routeNode)`

22.4. Добавить PHPDoc

### Результаты
- Централизованная авторизация

### Тесты после блока 22
**Feature:**
...
## Блок 23: Preview режим для Entry (защищённый endpoint)

### Задачи
23.1. Создать `app/Http/Controllers/Admin/V1/EntryPreviewController.php`:
...
23.2. Добавить маршрут в `routes/api_admin.php`:
   - `GET /api/v1/admin/entries/{entry}/preview`

23.3. Опционально: поддержка query-флага на публичном роуте:
   - `?preview=true` только для авторизованных пользователей с правом `view`

23.4. Добавить PHPDoc
...
## Блок 24: Endpoint entry-picker для конструктора

### Задачи
24.1. Расширить `EntryController` или создать отдельный endpoint:
...
24.2. Использовать `EntryResource` или создать `EntryPickerResource`:
...
## Блок 25: Тесты безопасности и валидации

### Задачи
25.1. Создать `tests/Feature/DynamicRoutes/SecurityTest.php`:
...
## Блок 26: Создание команды routes:dynamic-lint

### Задачи
26.1. Создать `app/Console/Commands/DynamicRoutesLintCommand.php`:
   ...
   - Проверки:
     ...
     - Некорректные `options` для `ENTRY_RESOLVER`

26.2. Вывод предупреждений:
   - Цветной вывод (info/warning/error)
   - Exit code: 0 если всё ОК, 1 если есть ошибки

26.3. Опционально: JSON-формат вывода для CI

26.4. Добавить PHPDoc
...
## Блок 28: E2E тесты интеграции

### Задачи
28.1. Создать `tests/Feature/DynamicRoutes/IntegrationTest.php`:
...
28.2. Создать `tests/Feature/DynamicRoutes/EntryIntegrationTest.php`:
   - Тест: режим A (жёсткое назначение) работает end-to-end
   - Тест: режим B (resolver) работает end-to-end
   - Тест: `template_override` влияет на рендеринг (если SSR)

28.3. Тесты производительности:
...
## Блок 29: Обновление документации

### Задачи
29.1. Обновить `docs/generated/README.md` (через `php artisan docs:generate`):
   - Добавить описание новых классов и endpoints

29.2. Создать/обновить `docs/routing-system.md`:
   - Описание DB-driven роутинга
   - Примеры использования
   - API endpoints для конструктора

29.3. Обновить `docs/entry-routing-integration.md`:
   - Отметить реализованные части
   - Добавить примеры использования

29.4. Обновить PHPDoc во всех новых классах:
   - Проверить соответствие сигнатур и документации

### Результаты
- Актуальная документация

### Тесты после блока 29
**Документационные:**
...
## Блок 30: Финальная проверка и чеклист

### Задачи
30.1. Выполнить полный набор тестов:
   - `php artisan test`
   - Проверить покрытие критических путей

30.2. Проверить соответствие чеклисту:
...
30.3. Проверить производительность:
   - Boot приложения с 500 узлами
   - Время регистрации маршрутов

30.4. Проверить безопасность:
   - Нет SQL injection
...
3. **Блоки 9-11**: Базовый регистратор и поддержка action_type
4. **Блоки 12-13**: Интеграция Entry (оба режима)
5. **Блоки 14-17**: Кэширование и интеграция в RouteServiceProvider
6. **Блоки 18-22**: Admin API и валидация
7. **Блоки 23-24**: Дополнительные возможности (preview, entry-picker)
8. **Блоки 25-28**: Тесты и качество
9. **Блоки 29-30**: Документация и финальная проверка
