...
test('validates unique rule with table name on create', function () {
    // Создаём пользователя с email для проверки уникальности
    ...
    $response->assertJsonValidationErrors(['content_json.email']);
});

test('validates unique rule with table and column on create', function () {
    User::factory()->create(['email' => 'existing@example.com']);
...
test('validates unique rule with except on update', function () {
    ...
    Path::factory()->create([
        ...
        'validation_rules' => [
            'unique' => [
                'table' => 'users',
                'column' => 'email',
                'except' => ['column' => 'id', 'value' => $existingUser->id],
            ],
        ],
    ]);
    ...
    $entry = Entry::factory()->create([
        ...
        'data_json' => ['email' => 'existing@example.com'],
    ]);

    // Обновляем с тем же email (разрешено для этого пользователя)
    $response = $this->actingAs($this->user)
        ->withoutMiddleware([\App\Http\Middleware\JwtAuth::class, \App\Http\Middleware\VerifyApiCsrf::class])
...
test('validates unique rule with where condition on create', function () {
    User::factory()->create(['email' => 'active@example.com', 'is_admin' => true]);
    ...
    Path::factory()->create([
        ...
        'validation_rules' => [
            'unique' => [
                'table' => 'users',
                'column' => 'email',
                'where' => ['column' => 'is_admin', 'value' => true],
            ],
        ],
    ]);
    ...
    // Email существует, но у не-админа (разрешено)
    $response = $this->actingAs($this->user)
        ->withoutMiddleware([\App\Http\Middleware\JwtAuth::class, \App\Http\Middleware\VerifyApiCsrf::class])
        ->postJson('/api/v1/admin/entries', [
        'post_type' => $postType->slug,
...
test('validates exists rule with table and column on create', function () {
    $referencedEntry = Entry::factory()->create();
    ...
    $response = $this->actingAs($this->user)
        ...
        'title' => 'Test Entry',
        'content_json' => [
            'related_entry' => $referencedEntry->id,
        ],
    ]);
...
test('validates exists rule with where condition on create', function () {
    ...
    $response = $this->actingAs($this->user)
        ...
        'title' => 'Test Entry',
        'content_json' => [
            'related_entry' => $publishedEntry->id,
        ],
    ]);

    $response->assertStatus(201);

    // ID неопубликованной записи
    $response = $this->actingAs($this->user)
        ->withoutMiddleware([\App\Http\Middleware\JwtAuth::class, \App\Http\Middleware\VerifyApiCsrf::class])
        ->postJson('/api/v1/admin/entries', [
        'post_type' => $postType->slug,
