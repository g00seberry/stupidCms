...
- ✅ Правильно преобразует простой путь `title` → `content_json.title`
- ✅ Правильно преобразует вложенный путь `author.name` → `content_json.author.name`
- ✅ Правильно обрабатывает cardinality='many' для родительского пути
  - `author.contacts.phone` с `author.cardinality='many'` → `content_json.*.contacts.phone`
- ✅ Правильно обрабатывает множественные уровни массивов
  - `items.tags.name` с `items.cardinality='many'` и `tags.cardinality='many'` → `content_json.*.*.name`
...
- ✅ `buildFieldPath()` добавляет префикс `content_json.` к простому пути
- ✅ `buildFieldPath()` обрабатывает путь без префикса (пустая строка)

### 2.2. Вложенные пути

- ✅ Правильно обрабатывает одноуровневую вложенность: `author.name` → `content_json.author.name`
- ✅ Правильно обрабатывает многоуровневую вложенность: `author.contacts.phone` → `content_json.author.contacts.phone`

### 2.3. Обработка cardinality

- ✅ Заменяет сегмент на `*` если родительский путь имеет `cardinality='many'`
  - `author.contacts` с `author.cardinality='many'` → `content_json.*.contacts`
- ✅ Правильно обрабатывает множественные уровни массивов
  - `items.tags.name` с `items.cardinality='many'` и `tags.cardinality='many'` → `content_json.*.*.name`
...
- ✅ Создание Entry с валидным content_json по правилам Blueprint
- ✅ Создание Entry с невалидным content_json (не проходит валидацию)
- ✅ Обновление Entry с валидным content_json
- ✅ Обновление Entry с невалидным content_json
...
- ✅ Пустой content_json
- ✅ Отсутствие content_json в запросе
...
- ✅ Правильно обрабатывает путь `level1.level2.level3.level4.level5.field` → `content_json.level1.level2.level3.level4.level5.field`
- ✅ Правильно обрабатывает путь с массивами на разных уровнях:
  - `items[].variants[].options[].values[].name` → `content_json.*.*.*.*.name`
  - Где `items`, `variants`, `options`, `values` имеют `cardinality='many'`
- ✅ Правильно обрабатывает смешанную структуру:
  - `events[].location.address.coordinates.lat` → `content_json.*.location.address.coordinates.lat`
  - Где `events` - массив, `location`, `address`, `coordinates` - объекты

#### 10.1.2. Массивы в массивах (nested arrays)

- ✅ Массив массивов строк: `tags[][].name` → `content_json.*.*.name`
- ✅ Массив массивов объектов: `sections[][].blocks[].content` → `content_json.*.*.*.content`
- ✅ Трёхуровневые массивы: `matrix[][][].value` → `content_json.*.*.*.value`
- ✅ Массивы с объектами внутри: `items[].metadata.tags[].name` → `content_json.*.metadata.*.name`

#### 10.1.3. Объекты в массивах (nested objects in arrays)

- ✅ Массив объектов с вложенными объектами:
  - `authors[].contact.address.street` → `content_json.*.contact.address.street`
- ✅ Массив объектов с массивами внутри:
  - `products[].categories[].name` → `content_json.*.*.name`
- ✅ Глубокая вложенность объектов в массивах:
  - `events[].venue.location.coordinates.lat` → `content_json.*.venue.location.coordinates.lat`

#### 10.1.4. Смешанные структуры (arrays + objects)

- ✅ Объект → массив → объект → массив:
  - `config.servers[].databases[].tables[].name` → `content_json.config.*.*.*.name`
- ✅ Массив → объект → массив → объект:
  - `items[].metadata.tags[].meta.key` → `content_json.*.metadata.*.meta.key`
- ✅ Чередование уровней: `a[].b.c[].d.e[].f` → `content_json.*.b.*.d.*.f`
...
- ✅ Путь с 3 wildcards: `a[].b[].c[].d` → `content_json.*.*.*.d`
- ✅ Путь с 5 wildcards: `a[].b[].c[].d[].e[].f` → `content_json.*.*.*.*.*.f`
- ✅ Путь с 10 wildcards подряд
- ✅ Чередование wildcards и обычных сегментов:
  - `a[].b.c[].d.e[].f` → `content_json.*.b.*.d.*.f`

#### 10.2.3. Сложные комбинации cardinality

- ✅ Все родители - массивы: `a[].b[].c[].d` → `content_json.*.*.*.d`
- ✅ Все родители - объекты: `a.b.c.d` → `content_json.a.b.c.d`
- ✅ Смешанные: `a[].b.c[].d.e` → `content_json.*.b.*.d.e`
- ✅ Первый уровень - массив, остальные - объекты: `items[].meta.key` → `content_json.*.meta.key`
- ✅ Последний уровень - массив: `config.items[].name` → `content_json.config.*.name`
...
- ✅ Валидация `content_json.items[][].name` (массив массивов)
- ✅ Валидация `content_json.matrix[][][].value` (трёхуровневый массив)
...
- ✅ Валидация `content_json.authors[].contact.email` (объект в массиве)
- ✅ Валидация `content_json.products[].metadata.tags[].name` (объект → массив в массиве)
...
- ✅ `required_if` на поле 5-го уровня вложенности
  - Пример: `content_json.events[].venue.location.coordinates.lat` required_if `content_json.events[].venue.location.type == 'gps'`
- ✅ `prohibited_unless` на поле в массиве массивов
- ✅ Условные правила, зависящие от полей на разных уровнях:
  - `content_json.items[].price` required_if `content_json.items[].category == 'premium'`
  - `content_json.sections[][].blocks[].content` required_if `content_json.sections[][].blocks[].type == 'text'`

#### 10.3.5. Сравнение полей через несколько уровней

- ✅ `field_comparison` между полями на разных уровнях:
  - `content_json.events[].end_date` >= `content_json.events[].start_date`
  - `content_json.items[].variants[].price` >= `content_json.items[].base_price`
- ✅ `field_comparison` между полями в разных массивах:
  - `content_json.orders[].total` >= `content_json.products[].min_price`
- ✅ `field_comparison` с константой на глубоко вложенном поле:
  - `content_json.config.servers[].port` >= 1024

#### 10.3.6. Комбинации всех правил на сложных структурах

- ✅ Все типы правил одновременно на глубоко вложенном поле:
  - `content_json.items[].variants[].options[].values[].name`:
    - required: true
    - min: 3
    - max: 100
    - pattern: '/^[a-z]+$/'
...
- ✅ Валидация с условными правилами, зависящими от других полей content_json
