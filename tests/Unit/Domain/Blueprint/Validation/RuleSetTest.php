...
test('addRule adds rule for new field', function () {
    ...
    $this->ruleSet->addRule('content_json.title', $rule);

    expect($this->ruleSet->hasRulesForField('content_json.title'))->toBeTrue();
    expect($this->ruleSet->getRulesForField('content_json.title'))->toHaveCount(1);
    expect($this->ruleSet->getRulesForField('content_json.title')[0])->toBe($rule);
});
...
test('addRule adds multiple rules for one field', function () {
    ...
    $this->ruleSet->addRule('content_json.title', $requiredRule);
    $this->ruleSet->addRule('content_json.title', $minRule);
    $this->ruleSet->addRule('content_json.title', $maxRule);

    $rules = $this->ruleSet->getRulesForField('content_json.title');
...
test('addRule adds rules for different fields', function () {
    ...
    $this->ruleSet->addRule('content_json.title', $titleRule);
    $this->ruleSet->addRule('content_json.description', $descriptionRule);

    expect($this->ruleSet->hasRulesForField('content_json.title'))->toBeTrue();
    expect($this->ruleSet->hasRulesForField('content_json.description'))->toBeTrue();
    expect($this->ruleSet->getRulesForField('content_json.title'))->toHaveCount(1);
    expect($this->ruleSet->getRulesForField('content_json.description'))->toHaveCount(1);
});
...
test('getRulesForField returns rules for existing field', function () {
    ...
    $this->ruleSet->addRule('content_json.title', $rule1);
    $this->ruleSet->addRule('content_json.title', $rule2);
...
test('getRulesForField returns empty array for non-existent field', function () {
    $rules = $this->ruleSet->getRulesForField('content_json.non_existent');
...
test('getAllRules returns all rules', function () {
    ...
    $this->ruleSet->addRule('content_json.title', $titleRule);
    $this->ruleSet->addRule('content_json.description', $descriptionRule);
    $this->ruleSet->addRule('content_json.author', $authorRule);

    $allRules = $this->ruleSet->getAllRules();

    expect($allRules)->toBeArray();
    expect($allRules)->toHaveKeys(['content_json.title', 'content_json.description', 'content_json.author']);
    expect($allRules['content_json.title'])->toHaveCount(1);
    expect($allRules['content_json.description'])->toHaveCount(1);
    expect($allRules['content_json.author'])->toHaveCount(1);
});

test('hasRulesForField returns true for field with rules', function () {
    $this->ruleSet->addRule('content_json.title', new RequiredRule());

    expect($this->ruleSet->hasRulesForField('content_json.title'))->toBeTrue();
});

test('hasRulesForField returns false for field without rules', function () {
    expect($this->ruleSet->hasRulesForField('content_json.title'))->toBeFalse();
});
...
test('getFieldPaths returns list of all field paths', function () {
    $this->ruleSet->addRule('content_json.title', new RequiredRule());
    $this->ruleSet->addRule('content_json.description', new MinRule(10));
    $this->ruleSet->addRule('content_json.author.name', new MaxRule(100));

    $paths = $this->ruleSet->getFieldPaths();

    expect($paths)->toBeArray();
    expect($paths)->toHaveCount(3);
    expect($paths)->toContain('content_json.title');
    expect($paths)->toContain('content_json.description');
    expect($paths)->toContain('content_json.author.name');
});
