...
test('getParams returns array with operator, other_field, constant_value', function () {
    ...
    expect($params['other_field'])->toBe('content_json.start_date');
...
test('correctly handles field comparison', function () {
    ...
    expect($rule->getOtherField())->toBe('content_json.start_date');
    expect($rule->getConstantValue())->toBeNull();
    expect($rule->getParams()['other_field'])->toBe('content_json.start_date');
...
test('correctly handles nested field paths', function () {
    $rule = new FieldComparisonRule('>=', 'content_json.author.birth_date', '2000-01-01');

    expect($rule->getOtherField())->toBe('content_json.author.birth_date');
    expect($rule->getParams()['other_field'])->toBe('content_json.author.birth_date');
});
