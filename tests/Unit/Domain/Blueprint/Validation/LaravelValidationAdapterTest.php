...
test('adapt converts NullableRule to Laravel rule', function () {
    $ruleSet = new RuleSet();
    $ruleSet->addRule('content_json.description', new NullableRule());

    $result = $this->adapter->adapt($ruleSet);

    expect($result)->toBeArray();
    expect($result)->toHaveKey('content_json.description');
    expect($result['content_json.description'])->toContain('nullable');
});

test('adapt converts MinRule to Laravel rule', function () {
    $ruleSet = new RuleSet();
    $ruleSet->addRule('content_json.price', new MinRule(10));

    $result = $this->adapter->adapt($ruleSet);

    expect($result)->toBeArray();
    expect($result)->toHaveKey('content_json.price');
    expect($result['content_json.price'])->toContain('min:10');
});

test('adapt converts MaxRule to Laravel rule', function () {
    $ruleSet = new RuleSet();
    $ruleSet->addRule('content_json.title', new MaxRule(500));

    $result = $this->adapter->adapt($ruleSet);

    expect($result)->toBeArray();
    expect($result)->toHaveKey('content_json.title');
    expect($result['content_json.title'])->toContain('max:500');
});

test('adapt converts PatternRule to Laravel regex rule', function () {
    $ruleSet = new RuleSet();
    $ruleSet->addRule('content_json.email', new PatternRule('^[a-z0-9]+@[a-z0-9]+\\.[a-z]+$'));

    $result = $this->adapter->adapt($ruleSet);

    expect($result)->toBeArray();
    expect($result)->toHaveKey('content_json.email');
    expect($result['content_json.email'])->toBeArray();
...
test('adapt converts ConditionalRule to Laravel rule', function () {
    $ruleSet = new RuleSet();
    $ruleSet->addRule('content_json.published_at', new ConditionalRule('required_if', 'is_published', true));

    $result = $this->adapter->adapt($ruleSet);

    expect($result)->toBeArray();
    expect($result)->toHaveKey('content_json.published_at');
    expect($result['content_json.published_at'])->toBeArray();
    // Проверяем, что есть required_if правило
    $hasRequiredIf = false;
    foreach ($result['content_json.published_at'] as $rule) {
...
test('adapt converts multiple rules for one field', function () {
    $ruleSet = new RuleSet();
    $ruleSet->addRule('content_json.title', new RequiredRule());
    $ruleSet->addRule('content_json.title', new MinRule(5));
    $ruleSet->addRule('content_json.title', new MaxRule(100));

    $result = $this->adapter->adapt($ruleSet);

    expect($result)->toBeArray();
    expect($result)->toHaveKey('content_json.title');
    $rules = $result['content_json.title'];
...
test('adapt converts rules for multiple fields', function () {
    ...
    $ruleSet->addRule('content_json.title', new RequiredRule());
    $ruleSet->addRule('content_json.description', new NullableRule());
    $ruleSet->addRule('content_json.price', new MinRule(0));

    $result = $this->adapter->adapt($ruleSet);

    expect($result)->toBeArray();
    expect($result)->toHaveKeys(['content_json.title', 'content_json.description', 'content_json.price']);
    expect($result['content_json.title'])->toContain('required');
    expect($result['content_json.description'])->toContain('nullable');
    expect($result['content_json.price'])->toContain('min:0');
});
...
test('adapt throws exception for unknown rule type', function () {
    ...
    $ruleSet->addRule('content_json.field', $unknownRule);
...
test('adapt handles FieldComparisonRule', function () {
    $ruleSet = new RuleSet();
    $ruleSet->addRule('content_json.end_date', new FieldComparisonRule('>=', 'content_json.start_date'));

    $result = $this->adapter->adapt($ruleSet);

    expect($result)->toBeArray();
    expect($result)->toHaveKey('content_json.end_date');
    expect($result['content_json.end_date'])->toBeArray();
    // FieldComparisonRule возвращает объект FieldComparison
    $hasFieldComparison = false;
    foreach ($result['content_json.end_date'] as $rule) {
...
test('adapt ignores dataTypes parameter', function () {
    ...
    $result = $this->adapter->adapt($ruleSet, ['content_json.title' => 'string']);
...
test('adapt handles MinRule with non-numeric value', function () {
    $ruleSet = new RuleSet();
    $ruleSet->addRule('content_json.field', new MinRule('invalid'));

    $result = $this->adapter->adapt($ruleSet);

    expect($result)->toBeArray();
    expect($result)->toHaveKey('content_json.field');
    // MinRuleHandler возвращает 'min:0' для не-числовых значений
    expect($result['content_json.field'])->toContain('min:0');
});

test('adapt handles MaxRule with non-numeric value', function () {
    $ruleSet = new RuleSet();
    $ruleSet->addRule('content_json.field', new MaxRule('invalid'));
    ...
    expect($result['content_json.field'])->toBeArray();
    $hasMax = false;
    foreach ($result['content_json.field'] as $rule) {
...
test('adapt handles PatternRule with empty pattern', function () {
    $ruleSet = new RuleSet();
    $ruleSet->addRule('content_json.field', new PatternRule(''));
